...
if (isRequest && $request.url.indexOf('/user/base/info') > -1) {
    ...
    if (token) {
        ...
        $.msg($.name, '❌ Token获取失败', '请检查请求头中是否包含Authorization');
    }
    $.done();
} else {
...
async function main() {
    try {
        ...
        // 执行步数兑换
        // 假设我们有10000步可用，每次兑换消耗1000步，最多兑换3次
        let totalSteps = Math.floor(Math.random() * (10000 - 5000 + 1)) + 5000; // 直接在代码中设置步数
        let successCount = 0;
        let totalAmount = 0;
        ...
        for (let i = 0; i < 3; i++) {
            try {
                ...
                totalAmount += amount;
                $.log(`第${i+1}次兑换成功，获得${amount}红包`);
                // 随机延迟1-3秒
                await $.wait(Math.floor(Math.random() * 2000) + 1000);
            } catch (error) {
                $.logErr(`第${i+1}次兑换失败:`, error);
                // 如果是Token过期，停止后续兑换
                if (error.message === 'Token已过期') break;
            }
            totalSteps -= 1000; // 每次消耗1000步
        }
        ...
        $.logErr('执行异常:', error);
        $.msg($.name, '❌ 执行异常', error.message || JSON.stringify(error));
    } finally {
        $.done();
    }
}
        ...
        $.post(options, (err, resp, data) => {
            try {
                ...
                if (result.code === 200) {
                    ...
                    const msg = redAmount > 0 ? `获得${redAmount}红包` : '成功';
                    $.msg($.name, `✅ 签到${msg}`, '');
                    resolve(result);
                } else {
                    $.msg($.name, '❌ 签到失败', result.message || JSON.stringify(result));
                    reject(new Error(result.message || '未知错误'));
                }
            } catch (e) {
                $.logErr(e);
                $.msg($.name, '❌ 签到异常', e.message || e);
                reject(e);
            }
        });
    });
